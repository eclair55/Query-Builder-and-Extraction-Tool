...
{
    public override string ProviderType => "PostgreSQL";
    ...
    public override async Task<IEnumerable<string>> GetSchemasAsync(DatabaseConnectionConfig config)
    {
        ...
        cmd.CommandText = "SELECT schema_name FROM information_schema.schemata WHERE schema_name NOT IN ('pg_catalog', 'information_schema') ORDER BY schema_name";
    ...
    public override async Task<IEnumerable<TableMetadataDto>> GetTablesAsync(DatabaseConnectionConfig config, string schema)
    {
        ...
        string sql = @"
            SELECT t.table_name, g.f_geometry_column, g.srid, g.type
            FROM information_schema.tables t
            LEFT JOIN geometry_columns g ON t.table_schema = g.f_table_schema AND t.table_name = g.f_table_name
        ...
        while (await reader.ReadAsync())
        {
            ...
            int srid = reader.IsDBNull(2) ? 4326 : reader.GetInt32(2);
            var geomType = reader.IsDBNull(3) ? (string.IsNullOrEmpty(geomCol) ? "" : "GEOMETRY") : reader.GetString(3);
    ...
    public override async Task<IEnumerable<ColumnMetadata>> GetColumnsAsync(DatabaseConnectionConfig config, string schema, string table)
    {
        ...
        while (await reader.ReadAsync())
        {
            ...
            bool isGeom = udtName.Equals("geometry", StringComparison.OrdinalIgnoreCase) || udtName.Equals("geography", StringComparison.OrdinalIgnoreCase);
    ...
    public override string BuildSpatialPredicate(SpatialQueryDefinition spatial, string geomColumn, Dictionary<string, object> parameters, int sourceSrid)
    {
        ...
        if (sourceSrid != 0 && targetSrid != sourceSrid)
        {
            targetGeomSql = $"ST_Transform({targetGeomSql}, {sourceSrid})";
        }
        ...
        switch (op)
        {
            ...
            case "WITHIN_DISTANCE":
                ...
                return $"ST_DWithin({geomColumn}::geography, {targetGeomSql}::geography, @p_dist)";
            case "BUFFER":
                double bufMeters = ConvertToMeters(spatial.Distance ?? 0, spatial.Unit);
                parameters["@p_buf"] = bufMeters;
                return $"ST_Intersects({geomColumn}, ST_Buffer({targetGeomSql}::geography, @p_buf)::geometry)";
            default:
                return $"ST_Intersects({geomColumn}, {targetGeomSql})";
        }
    }
